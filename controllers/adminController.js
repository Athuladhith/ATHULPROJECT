  ...
  try{
    ...
    await Offer.findByIdAndUpdate(blockid,{$set:{status:!block_status}},
     ...
     const latest = lateststatus.status
      ...
      if (latest==false) {
        console.log("entered")
        const discountPercentage = offerdata.discount; // Percentage discount
        ...
        for (const product of productsToUpdate) {
          ...
          const discountAmount = (originalPrice * discountPercentage) / 100; // Calculate discount amount
          const updatedPrice = originalPrice - discountAmount;
          console.log(updatedPrice,"updated price")
          // Update product price and unblock it
          await Product.updateMany({category:cat}, { $set: { price: updatedPrice} });
        }
      }else if(latest==true){
        const productsToUpdate = await Product.find({ category: cat });
